...
L.NonTiledLayer = L.Layer.extend({
    ...
    onAdd: function (map) {
        this._map = map;
    	map.fire("dataloading", null); //this is necessary as the layer is added and the 'loading' event is fired before the event has had the handler attached that shows the loading icon (in the Control.Loading.js module)
        if (!this._div) {
    ...
    _update: function () {
        ...
        this.fire('loading', null); //added to be able to use the Control.Loading.js class
        if (this.getImageUrl)
	...
	_onImageDone: function (success, e) {
        ...
        this.fire('load', e);
    	this._map.fire("dataload", null); //this is necessary as the layer is added and the 'loading' event is fired before the event has had the handler attached that shows the loading icon (in the Control.Loading.js module)
    }
});
