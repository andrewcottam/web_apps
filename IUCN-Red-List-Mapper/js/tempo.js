...
var Tempo = (function(tempo){
        ...
        parse: function(container){
            ...
            for (var i = 0; i < children.length; i++) {
                if (children[i].getAttribute !== undefined && children[i].getAttribute('data-template') !== null && (this.nestedItem === children[i].getAttribute('data-template') || children[i].getAttribute('data-template') === '' && !utils.isNested(children[i]))) {
                    ...
                    if (children[i].getAttribute !== undefined && children[i].getAttribute('data-template-fallback') !== null) {
        ...
        createTemplate: function(node){
            ...
            for (var a = 0; a < element.attributes.length; a++) {
                ...
                if (utils.startsWith(attr.name, 'data-if-')) {
                    ...
                    if (attr.value === 'notnull') {
                        this.namedTemplates[attr.name.substring(8, attr.name.length) + '!=null'] = element;
                    }
                    else {
    ...
    Renderer.prototype = {
        ...
        tags: [ // If tag (using a workaround for the dotall modifier to match multilines [\\s\\S])
        {
            'regex': '\\{\\{if ([\\s\\S]*?)\\}\\}([\\s\\S]*?)\\{\\{endif\\}\\}',
