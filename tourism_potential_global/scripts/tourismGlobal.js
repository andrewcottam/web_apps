require(["dojo/on", "esri/map", "esri/graphic", "esri/symbols/SimpleMarkerSymbol", "esri/symbols/SimpleLineSymbol", "esri/request", "dgrid/OnDemandGrid", "dojo/_base/Color", "esri/InfoTemplate", "dojo/_base/array", "esri/geometry/Point", "esri/SpatialReference", "esri/symbols/TextSymbol", "esri/symbols/Font", "dijit/registry", "dojo/ready", "dojo/Evented", "dojo/_base/declare", "dijit/form/Select", "dojo/parser", "dijit/form/CheckBox", "dijit/form/TextBox", "dijit/form/Button", "dojo/promise/all", "dojo/Deferred", "dojo/query", "dojo/dom", "dojo/dom-style", "dojo/dom-construct", "dijit/layout/BorderContainer", "dijit/layout/ContentPane", "dgrid/Selection", "dojo/store/Memory"], function(on, Map, Graphic, SimpleMarkerSymbol, SimpleLineSymbol, esriRequest, OnDemandGrid, Color, InfoTemplate, arrayUtil, Point, SpatialReference, TextSymbol, Font, registry, ready, Evented, declare, Select, parser, CheckBox, TextBox, Button, all, Deferred, query, dom, domStyle, domConstruct, BorderContainer, ContentPane, Selection, Memory) {
	ready(function() {
		...
		sitesGrid = declare([OnDemandGrid, Selection])({
			columns : [{
				...
				field : "flickr_raw_sum_rank",
				label : "total",
				sortable : false
			}, {
				field : "flickr_ln_sum_rank",
				label : "ln",
				sortable : false
			}, {
				field : "flickr_sqrt_sum_rank",
				label : "sqr",
				sortable : false
			}, {
				field : "flickr_avg_rank",
				label : "avg",
				sortable : false
			}, {
				field : "species_richness_rank",
				label : "rich",
				sortable : false
			}, {
				field : "red_list_sum_rank",
				label : "status",
				sortable : false
			}, {
				field : "charisma_sum_rank",
				label : "score",
				sortable : false
			}],
		...
		var requestDeferred = requestData(10);
	...
	var map;
	var restServerUrl = "https://61c92e42cb1042699911c485c38d52ae.vfs.cloud9.eu-west-1.amazonaws.com:8080/python-rest-server/h05googleearthengine/";
	var sitesGrid;
	var speciesGrid;
	var currentRow;
	...
	function requestData(records) {
		...
		requestDeferred = esriRequest({
			url : restServerUrl + "/especies/_get_tourism_ranks_global",
			content : {
				sortby : tp_field,
				limitto : records,
				fields: 'country,wdpa_id,name,centroid,flickr_raw_sum_rank,flickr_ln_sum_rank,flickr_sqrt_sum_rank,flickr_avg_rank,species_richness_rank,red_list_sum_rank,charisma_sum_rank'
			},
			handleAs : "json",
	...
	function createMarkers(records) {
		...
		var infoTemplate = new InfoTemplate("${" + tp_field + "}: ${name}", "Total charisma: ${flickr_raw_sum_rank}<br />Average Charisma: ${flickr_avg_rank}<br />Ln Charisma: ${flickr_ln_sum_rank}<br />Sqr Charisma: ${flickr_sqrt_sum_rank}<br />Species Richness: ${species_richness_rank}<br />Total Red List Score: ${red_list_sum_rank}<br />Total Charisma Score: ${charisma_sum_rank}<br />");
