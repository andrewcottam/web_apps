...
const App: React.FC = () => {
  ...
  const analyzeDisturbance = async () => {
    ...
    try {
      ...
      // Convert to GeoJSON geometry object expected by the linear-regression API
      let geojsonGeometry: Record<string, any>;
      if (geometry4326 instanceof MultiPolygon) {
        geojsonGeometry = {
          type: "MultiPolygon",
          coordinates: geometry4326.getPolygons().map(poly =>
        ...
        geojsonGeometry = {
          type: "Polygon",
          coordinates: [coordinates.map((coord: number[]) => [coord[0], coord[1]])]
        };
      }
      ...
      const endpoint = isLocalhost
        ? "http://localhost:8080/linear-regression"
        : "https://europe-west6-restor-gis.cloudfunctions.net/linear-regression";
      ...
      alert(`Failed to analyze change:\n\n${errorMessage}`);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="panel">
        ...
        {logged_in && (
          <div className="panel-content">
            ...
            <h2>Change in vegetation</h2>

            <p style={{ fontSize: "13px", color: "#666", margin: "10px 0" }}>
              Draw a polygon on the map to analyze vegetation change
            </p>
                ...
                {data.response?.metadata?.report_url && (
                  <div style={{ marginTop: "10px", padding: "8px", backgroundColor: "#e3f2fd", borderRadius: "4px" }}>
                    ...
                    <p style={{ fontSize: "12px", margin: "4px 0" }}>
                      <a href={data.response.metadata.report_url} target="_blank" rel="noopener noreferrer">
                        View Report
                      </a>
                    </p>
                  </div>
